...
void axpy_intrinsics(double a, double *dx, double *dy, int n) {
  ...
  long gvl = __builtin_epi_vsetvl(n, __epi_e64, __epi_m1); // TODO: Highly illegal: We are supposed to use the intrinsics
  // long gvl = vsetvl_e64m1(n); //PLCT
  
  _MMR_f64 v_a = _MM_SET_f64(a, gvl);
  
  for (i = 0; i < n;) {
    gvl = __builtin_epi_vsetvl(n - i, __epi_e64, __epi_m1); // TODO: Highly illegal: We are supposed to use the intrinsics
