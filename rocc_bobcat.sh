...
#  "make -C sims/verilator run-binary-debug-hex CONFIG=RocketVecRoccExampleConfig BINARY=$PWD/tests/rvv/riscv-vectorized-benchmark-suite/_axpy/bin/axpy_vector.exe EXTRA_SIM_FLAGS=--verbose"
  "make -C sims/verilator run-binary-hex CONFIG=RocketVecRoccExampleConfig BINARY=$PWD/tests/rvv/kernels/_axpy/bin/axpy_vector.exe EXTRA_SIM_FLAGS=--verbose"
  "make -C sims/verilator run-binary-hex CONFIG=SmallBobcatConfig BINARY=$PWD/tests/rvv/kernels/_axpy/bin/axpy_vector.exe EXTRA_SIM_FLAGS=--verbose"
)
